...
createPublicKey = (input) => {
    if (input instanceof KeyObject) {
        return createPublicKey({ key: asn1.get('RSAPublicKey').encode(input.asn1), format: 'der', type: 'pkcs1' })
    }
    ...
    switch (type) {
        case 'spki': {
            const parsed = asn1.get('PublicKeyInfo').decode(key, format, { label })
            let keyObject
            switch (parsed.algorithm.algorithm.join('.')) {
                //!! case '1.2.840.113549.1.1.1': {
                default: {
                    keyObject = createPublicKey({ type: "pkcs1", key: parsed.publicKey.data, format: 'der' })
                    break
                }
            }
            return keyObject
        ...
        //!! case 'pkcs1': {
        default: {
            let RSAPublicKey = asn1.get('RSAPublicKey');
